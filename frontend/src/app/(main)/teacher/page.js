...
const usePagination = (url, { itemsPerPage = 6 } = {}) => {
  ...
  const [meta, setMeta]       = useState({
    currentPage: 1, totalPages: 1, totalItems: 0, itemsPerPage,
  });
  ...
  const fetchData = useCallback(async ({
    ...
    try {
      ...
      const rows        = d.teachers ?? d.bundles ?? d.data ?? d.rows ?? d.quizzes ?? [];
      const totalItems  = d.totalRecords ?? d.pagination?.totalItems ?? d.total ?? 0;
      const totalPages  = d.totalPages   || d.pagination?.totalPages || d.pages || Math.ceil(totalItems / limit) || 1;
      const currentPage = d.currentPage  ?? d.pagination?.currentPage ?? page;
      setData(rows);
  ...
  return {
    ...
    totalItems: meta.totalItems,   itemsPerPage: meta.itemsPerPage,
    pageNumbers,
...
// ─── Reusable Field components (defined OUTSIDE main component) ───────────────
// This is critical — defining these inside the component causes remount on every
// render, which resets focus after each keystroke.
...
export default function TeachersCRUD() {
  const {
    ...
    fetchData, goToPage,
  } = usePagination(API_URL, { itemsPerPage: 6 });
  ...
  // Fetch on page / search / sort change
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      closeModal();
      fetchData({ page: editingId ? currentPage : 1, search: debouncedSearch, sortBy, sortOrder });
    } catch (err) {
  ...
  return (
    <div style={{ padding: "30px 32px", backgroundColor: "#f8fafc", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "22px", gap: "12px", flexWrap: "wrap" }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "21px", fontWeight: 700, color: "#0f172a", fontFamily: "'DM Serif Display', serif", letterSpacing: "-0.02em" }}>
            Teachers
          </h1>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "9px" }}>
          ...
          <div style={{
            ...
            background: "#fff",
            border: `1.5px solid ${searchOpen ? "#3b82f6" : "#e2e8f0"}`,
            borderRadius: "10px", overflow: "hidden", height: "38px",
            width: searchOpen ? "230px" : "38px",
            ...
            <button
              ...
              onClick={() => {
                setSearchOpen((o) => !o);
                if (searchOpen) { setSearchInput(""); goToPage(1); }
              }}
            ...
            <input
              ...
              type="text"
              value={searchInput}
              onChange={(e) => { setSearchInput(e.target.value); goToPage(1); }}
              placeholder="Search teachers…"
              style={{
                border: "none", outline: "none", fontSize: "13px", color: "#111827",
                width: "100%", paddingRight: "10px", background: "transparent",
          ...
          <select
            ...
            onChange={(e) => {
              const [col, order] = e.target.value.split("__");
              setSortBy(col); setSortOrder(order); goToPage(1);
            }}
      ...
      <div style={{ background: "#fff", borderRadius: "14px", border: "1px solid #e8ecf4", overflow: "hidden" }}>
        {error && (
          <div style={{ padding: "14px 22px", background: "#fef2f2", borderBottom: "1px solid #fecaca", color: "#dc2626", fontSize: "13px", display: "flex", alignItems: "center", gap: "7px" }}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path strokeLinecap="round" d="M12 8v4m0 4h.01"/></svg>
            {error}
          </div>
        )}
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} style={{ padding: "64px", textAlign: "center", color: "#94a3b8", fontSize: "14px" }}>
                  ...
                  No teachers found{searchInput ? ` for "${searchInput}"` : ""}
                </td>
              </tr>
            ) : (
              teachers.map((t, i) => {
                ...
                return (
                  <tr key={t.id ?? i} className="tr-row" style={{ borderBottom: "1px solid #f1f5f9", transition: "background 0.12s" }}>
                    <td style={{ padding: "14px 18px", fontSize: "12px", color: "#94a3b8", fontWeight: 600 }}>
                      {String((currentPage - 1) * 6 + i + 1).padStart(2, "0")}
                    </td>
        ...
        {!loading && totalPages > 1 && (
          <div style={{ padding: "14px 22px", borderTop: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <span style={{ fontSize: "12.5px", color: "#64748b" }}>
              Showing{" "}
              <b style={{ color: "#0f172a" }}>{(currentPage - 1) * 6 + 1}–{Math.min(currentPage * 6, totalItems)}</b>
              {" "}of <b style={{ color: "#0f172a" }}>{totalItems}</b>
            </span>
